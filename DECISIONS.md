...
## 2026-08-20 — Location search requires picking a suggestion — Accepted
**Context:** The homepage hero and `/kennels` had drifted into two separate search implementations, and both silently returned zero results for common cases: "use my location" wrote the full reverse-geocoded address (Nominatim's `display_name`, e.g. "10 Downing Street, Westminster, ... United Kingdom") into the text box, and clicking an autocomplete suggestion discarded the `lat`/`lon` it carried and kept only its display text. Either way, `/kennels` received a free-text string that its `resolveLocation` cascade (bare postcode / outcode / single place name only) couldn't parse. Typing a full address and hitting enter failed the same way, for the same reason.
**Decision:** Location search now works like Rightmove/Zoopla: the visible text in the box is just a label, never re-parsed. Search is only submittable once the user has picked a suggestion (or used "use my location") — each of those carries real `lat`/`lng` through untouched to `/kennels?lat=&lng=&label=`. Editing the text after a pick clears the selection and re-requires a pick. Both entry points now share one component, `components/location-picker.tsx`, instead of two hand-rolled copies (`components/kennels-search-bar.tsx` wraps it for `/kennels`).
**Rationale:** The bug was structural, not a parsing edge case — no code path ever carried a resolved location from "user picked/found a place" through to the actual search. Forcing a pick (rather than adding a smarter server-side text parser) matches how established location-search products avoid this class of bug entirely, and building one shared component prevents the homepage/`/kennels` implementations from drifting again.
**Consequences:** `/kennels` accepts `lat`/`lng`/`label` as its primary search contract; `resolveLocation(q)` (`lib/geo/resolve-location.ts`) is kept only as a fallback for old bookmarked `?q=` links, unchanged otherwise. `app/demo/DemoPageClient.tsx` still has the old duplicated, unfixed Nominatim logic — it posts to a separate mock `/demo/results` route unrelated to real organisation search, so it was left alone; `LocationPicker` is reusable there if it's ever wired up. No regression test was added — consistent with the rest of the repo, which has no test suite wired up yet (see §9 of `AGENTS.md`).
